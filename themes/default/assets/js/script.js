...
jQuery(function($) {
	...
	var UI = {
		mMenu: function(){
			...
			m_nav.find('.nav-ct').append($('.h-account').clone());
			m_nav.find('.nav-ct').append($('.exit_mobile').clone().show());
			m_nav.find('.nav-ct').append($('.h-register').clone());
		    ...
		    win.on('resize orientationchange', function(e) {
		    	if(win.width() > 1199) body.css('overflow', '');
		    })
		},
	...
	$('.fix-right .m-open').click(function(e) {
		e.preventDefault();
		if(win.width()<768){
			$('.fix-right').toggleClass('active');
		}
	});
